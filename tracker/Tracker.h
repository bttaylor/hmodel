...
#include <FaceTracker/FTracker.h>
...
public:
	Tracker(Worker*worker, double FPS, std::string sequence_path, bool real_color, bool myoEnable, bool record_only, int user_name) : worker(worker), hub(hub), model("C:/Developer/FaceTracker-opencv2/model/face2.tracker") {
		setSingleShot(false);
	...
	Tracker(Worker*worker, myo::Hub* hub, double FPS, std::string sequence_path, bool real_color) : worker(worker), hub(hub) {
		setSingleShot(false);
		setInterval((1.0 / FPS)*1000.0);
		this->sequence_path = sequence_path;
		this->real_color = real_color;
...
public:
		...
		//TICTOC_BLOCK(uploading_time, "GPU uploading")
		{
			...
			if (mode == LIVE) {
				...
				if (r > .975 && !classified) {
					...
					Eigen::Matrix3f mat;
					mat = Eigen::AngleAxisf(theta[3], Eigen::Vector3f::UnitX()) * Eigen::AngleAxisf(theta[4], Eigen::Vector3f::UnitY()) * Eigen::AngleAxisf(theta[5], Eigen::Vector3f::UnitZ());
					...
					theta.push_back(acos(y_off[1]));
					theta.push_back(acos(x_off[1]));
					theta.push_back(acos(z_off[1]));
					//cout << "Fingers off Vert: " << acos(y_off[1])*180/ 3.1416 << " Thumb off: " << acos(x_off[1]) * 180 / 3.1416 << " Palm off: " << acos(z_off[1]) * 180 / 3.1416 << endl;
					int hs_class = worker->classify(theta);
					if (hs_class == 0) {
						cout << endl;
						word_guess = "";
					}
	...
	void next_participant() {
		...
		Worker new_worker(worker->camera, worker->test, worker->benchmark, worker->save_rastorized_model, new_user_name, worker->data_path, left_hand);
